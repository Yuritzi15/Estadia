<?php
	...
	Class CtrlGuardarInv{
		...
		static public function Guardar(){
			...
			if (isset($_POST['txt_nombre']) || isset($_POST['txt_cantidadP']) || isset($_POST['txt_precioC']) || isset($_POST['txt_precioV']) || isset($_POST['Cmb_cate'])  || isset($_POST['Cmb_Pr'])){
				...
				if(preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ]+$/',$_POST['txt_nombre'] || preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ]+$/',$_POST['txt_cantidadP']))){
					...
					"PrecioV" => $_POST["txt_precioV"], "Cate_id" => $_POST['Cmb_cate'], "Proveedor_id" => $_POST['Cmb_Pr']);
		...
		static public function Cmb_Pr(){
			$res = ModeloGuardarP::mdlCmb_Pr();
			return $res;
		}
	    ...
	    static public function GuardarPr(){
			if (isset($_POST['txt_nombre'])){
				if(preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ]+$/',$_POST['txt_nombre'])){
					$tabla = "proveedor";
					$data = array("Proveedor" => $_POST["txt_nombre"]);

					$res = ModeloGuardarP::mdlGuardarPr($tabla,$data);
		...
		static public function EliminarPr(){
			if(isset($_POST['txt_id'])){
				if(preg_match('/^[1-9][0-9]*$/',$_POST['txt_id'])){

					$tabla = "proveedor";
					$data = array("Id" => $_POST["txt_id"]);
					$res = ModeloGuardarP::mdlEliminarPr($tabla,$data);
					if($res = "active"){
						echo'<script>
								Swal.fire({
  								icon: "success",
